...
//
// Читая правило как список условий, соединённых «и», пустой список
// полагалось бы считать выполненным: невыполненных условий в нём нет.
// Здесь это НЕ так, и разница стоила красного сторожа: группа с пустым
// правилом, заведённая под кафедру, приняла бы в себя всех врачей
// установки разом, а привязанный к ней платный набор стал бы открыт
// каждому. Проверка поймала это словами «в группу с пустым правилом
// попал непричастный».
//
// Пустое правило означает не «все», а «правила нет»: такая группа
// держится поимённым списком, и это ровно то, что нужно кафедре — общего
// признака у её ординаторов нет. Решается это здесь, в одном месте, а не
// у каждого, кто зовёт Matches: второе место для этого правила однажды
// прочитало бы пустой список как выполненный.
