...
func TestПризнакиСчитаютсяПоПортрету(t *testing.T) {
	for _, случай := range []struct {
		...
		{
			// Не «все», а «правила нет»: такая группа держится
			// поимённым списком. Прочитанное как «все», пустое правило
			// раздало бы платный набор каждому врачу установки — и
			// ровно это однажды показал сторож.
			"пустое правило не берёт никого",
			Rule{}, портрет(func(*Portrait) {}), false,
		},
		{
			"почта привязана",
