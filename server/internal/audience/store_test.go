...
func TestPgСломанноеПравилоНеПрименяетсяНиВТуНиВДругуюСторону(t *testing.T) {
	...
	// Но поимённый список остаётся в силе: он не часть правила и от его
	// поломки не зависит. Кафедра из двухсот ординаторов, у которой
	// кто-то испортил правило руками в базе, не должна лишиться доступа
	// разом и молча — названа она поимённо и понята полностью.
	названный := врач(t, gate)
	если(t, store.AddMember(ctx, slug, названный, "operator"))
	его, err := store.Membership(ctx, названный, time.Now())
	если(t, err)
	if !входит(его, one.ID) {
		t.Error("поломка правила отобрала группу у названного поимённо")
	}

	если(t, store.DropMember(ctx, slug, названный))
