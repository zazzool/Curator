...
// live читает группы — все, но правило берёт только разобранное.
//
// # Неразобранное правило не применяется ни в ту, ни в другую сторону
//
// Это выбор, а не умолчание. Применить такое правило «как открывающее»
// значило бы раздать по тому, чего мы не поняли; «как скрывающее» —
// отнять по нему же. Набор остаётся при своей линейке — состоянии,
// которое кто-то назвал вслух, — а студия показывает поломку красной
// строкой, чтобы та не стояла молча.
//
// # А поимённый список при этом остаётся в силе
//
// Он не часть правила и от его поломки не зависит: список — это прямо
// высказанная воля оператора, и читать её нечем не надо. Выбрось мы
// группу целиком — кафедра из двухсот ординаторов, у которой кто-то
// испортил правило руками в базе, лишилась бы доступа разом и молча,
// хотя названа она поимённо и понята полностью.
//
// Достигается это тем, что у сломанной группы правило остаётся пустым, а
// пустое правило не берёт никого (см. Rule.Matches). Второго условия для
// этого не заводится: оно и было бы тем вторым местом, где решение
// однажды окажется другим.
