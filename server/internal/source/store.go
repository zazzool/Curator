...
// появившееся строкой в коде, прошло бы проверку схемы и осталось бы
// невидимым для всех списков.
const (
	StatusDraft   = "draft"
	StatusActive  = "active"
...
//
// # Зачем это отдельное действие, а не следствие приёмки
//
// Приёмка разбора говорит «вот что написано в источнике». Объявление
// действующим говорит «по этому источнику теперь учат врача», и это разные
// решения, принимаемые в разное время: разбор принимают по частям, документ
// за документом, а объявляют источник один раз и целиком. Сделай второе
// следствием первого — и первый же принятый документ выкатил бы врачам
// половину справочника, причём молча.
//
// Умолчание при заведении — черновик, и до этого вызова источника для
// устройства не существует: ни в списке справочников, ни в раздаче.
//
// # Пустой источник действующим не объявляется
//
// Отказ, а не молчаливое согласие: объявленный действующим источник без
// единой единицы врач увидит как пустую книгу и решит, что приложение
// сломано. Сломано при этом не приложение, а решение, и сказать об этом
// надо тому, кто его принимает, — здесь и сейчас.
func (s *Store) SetStatus(ctx context.Context, id int64, status string) error {
	switch status {
	case StatusDraft, StatusActive, StatusRetired:
	default:
		return fmt.Errorf("состояния %q не бывает: черновик, действующий или отменённый", status)
	}

	if status == StatusActive {
		var units int
		err := s.gate.QueryRow(ctx,
			`SELECT COUNT(*) FROM source_units WHERE source_id = $1`, id).Scan(&units)
		if err != nil {
			return fmt.Errorf("единицы источника %d не сосчитаны: %w", id, err)
		}
		if units == 0 {
			return fmt.Errorf("в источнике нет ни одной единицы: примите разбор хотя бы одного документа, и источник можно будет объявить действующим")
		}
	}

	tag, err := s.gate.Exec(ctx, `UPDATE sources SET status = $2 WHERE id = $1`, id, status)
	if err != nil {
		return fmt.Errorf("состояние источника %d не записано: %w", id, err)
	}
	if tag.RowsAffected() == 0 {
		return fmt.Errorf("источника %d нет", id)
	}
	return nil
}
