...
// maxDocumentXMLBytes — потолок РАЗВЁРНУТОЙ разметки документа.
//
// Сам файл ограничен MaxDocumentBytes, но это размер СЖАТЫЙ: zip с
// высокой степенью сжатия разворачивает гигабайты в контейнер, которому
// отведено 512 мегабайт. Падает при этом служба целиком — вместе с
// раздачей задач врачам, которые ничего не загружали.
//
// Двести мегабайт: разметка вчетверо-впятеро объёмнее текста, и приказ на
// тысячу страниц укладывается в десятки. Двести — это «такого документа
// не бывает», а не «нам лень».
const maxDocumentXMLBytes int64 = 200 << 20
...
func DocxToMarkdown(data []byte) (string, error) {
	return docxToMarkdown(data, maxDocumentXMLBytes)
}

// docxToMarkdown — то же с назначаемым потолком.
//
// Потолок отделён от константы ради проверки, и отделён именно так, а не
// изменяемой переменной пакета: собрать настоящую бомбу на двести
// мегабайт — это двадцать секунд у сторожа на каждый push, а проверяется
// в ней устройство, а не само число. С маленьким потолком бомба остаётся
// настоящей и идёт мгновенно.
func docxToMarkdown(data []byte, max int64) (string, error) {
	...
	for _, f := range zr.File {
		...
		// Названный размер сверяется до открытия, а прочитанное — во
		// время чтения. Одной сверки мало: размер в заголовке zip пишет
		// тот, кто собрал файл, и он волен соврать. Одного потолка при
		// чтении мало иначе: разворачивать гигабайт, чтобы на нём
		// отказать, — это те же гигабайты работы.
		if f.UncompressedSize64 > uint64(max) {
			return "", fmt.Errorf(
				"разметка документа больше %d МБ: похоже, это не текст приказа",
				max>>20)
		}
		rc, err := f.Open()
		if err != nil {
