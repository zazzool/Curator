...
// ClientsShown приводит спрошенный предел к тому, сколько клиентов
// ручка отдаст на самом деле.
//
// Отдельной функцией, а не правкой внутри Find: ручке нужно знать тот же
// предел, чтобы спросить на одного больше и отличить «их ровно
// пятьдесят» от «их больше пятидесяти». Считай она его по-своему —
// пятьдесят первый врач остался бы невидимым, а «показаны первые
// пятьдесят» не написалось бы, потому что ручка решила бы, что показала
// всех.
func ClientsShown(asked int) int {
...
func (c *Clients) Find(ctx context.Context, query string, limit int) ([]Client, error) {
	// Здесь предел берётся как названо, а приводит его ручка (ClientsShown).
	// Второе приведение тут было бы не осторожностью, а ошибкой: ручка
	// спрашивает на одного БОЛЬШЕ своего предела, и при пределе в самый
	// потолок это «больше» приводилось бы обратно к пятидесяти — поиск
	// молча сужался бы вчетверо там, где оператор просил шире всего.
	if limit <= 0 {
		limit = ClientsShown(0)
	}
