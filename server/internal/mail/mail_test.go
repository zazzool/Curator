...
func TestПереводСтрокиВАдресеНеПодставляетЗаголовок(t *testing.T) {
	// Адрес с переводом строки вписывает в письмо чужой заголовок, и
	// почтовый сервер исполняет его как свой: «Bcc» уводит копию письма
	// с кодом возврата доступа тому, кто его подставил.
	letter := string(Build(
	...
	if strings.Contains(letter, "Bcc:") && strings.Contains(letter, "\r\nBcc:") {
		t.Fatalf("в письме появился подставленный заголовок:\n%s", letter)
	}
	// Заголовков ровно столько, сколько написала Build: подделка не
	// добавила своего.
	head, _, _ := strings.Cut(letter, "\r\n\r\n")
	if got := strings.Count(head, "\r\n"); got != 5 {
		t.Errorf("заголовков %d вместо шести:\n%s", got+1, head)
	}
}
