...
const годныйОтвет = `{"title":"Срок рассмотрения",
	...
	"explanationMd":"Срок считается рабочими днями.","difficulty":3}`

// Порядок узлов конвейера — он же порядок ответов подставной модели.
//
// Именами, а не числами по месту: узел добавляется в середину (так пришла
// вычитка), и разъехавшиеся по файлу индексы после этого проверяют не тот
// запрос, оставаясь на вид зелёными.
const (
	узелНаписания = iota
	узелВычитки
	узелСверки
	узелСоседей
	// Различающая сверка идёт ПО КАЖДОМУ неверному варианту, а круг из
	// двух вариантов задачей не является: соседей в нём всегда несколько,
	// и обращений к модели столько же.
	узелВторогоСоседа
	узловВсего
)

// Узлы СПОРНОГО прогона, где слепая сверка не сошлась.
//
// Счётов у конвейера два, а не один, и это не удобство записи. Разбор
// расхождения спрашивают только при расхождении — сошедшаяся сверка
// второго вопроса не требует и денег не стоит, — а встаёт он МЕЖДУ
// слепой сверкой и различающей и сдвигает всё, что ниже. Сложи мы оба
// случая в один счёт, и пропажа целого узла на сошедшемся прогоне
// прошла бы молча, а спорный сверялся бы с чужим запросом.
const (
	узелРазбора   = узелСверки + 1
	узловСоСпором = узловВсего + 1
)

// ответВычитки — «замечаний нет»: пустой заголовок и пустой список
// фрагментов. Законный ответ редактора, оставляющий условие как есть, —
// проверкам конвейера нужен именно он, иначе они мерили бы текст,
// переписанный подставной моделью.
const ответВычитки = `{"title":"","segments":[]}`

// ответСоседа — чем отвечает подставная модель на узле различающей сверки.
//
// Он нужен всякой проверке, прогоняющей конвейер целиком: круг годного
// ответа содержит соседа 3.2, и различающая сверка спрашивает про него.
// Не заготовь ответа — узел молча отказал бы, а проверка осталась бы
// зелёной, прогнав сломанный конвейер.
const ответСоседа = `{"verdict":"contradicts","why":"условие о сроке об оформлении отказа не говорит"}`

// ответРазбора — чем отвечает подставная модель на узле разбора
// расхождения.
//
// Нужен всякой проверке, где слепая сверка НЕ сошлась. Не заготовь его —
// и разбор съел бы ответ, приготовленный различающей сверке: очередь у
// подставной модели одна, и съехавшая на узел вниз проверка мерила бы
// чужой запрос, оставаясь на вид зелёной.
//
// «Не решить», а не «заказанная точнее»: расхождение при этом остаётся
// расхождением, и проверки несогласия мерят то же, что мерили.
const ответРазбора = `{"verdict":"unclear","why":"по тексту решающего признака нет"}`
...
func TestPgСлепаяСверкаОстаётсяСлепой(t *testing.T) {
	...
	// Счёт точный, а не «хотя бы столько»: исчезнувший узел иначе прошёл
	// бы молча, а сверка, съехавшая на другое место, сверялась бы ниже не
	// с тем текстом.
	if len(model.спрошено) != узловСоСпором {
		t.Fatalf("узлов прошло %d вместо %d", len(model.спрошено), узловСоСпором)
	}
	сверка := model.спрошено[узелСверки].System + "\n" + model.спрошено[узелСверки].User
...
func TestPgНесостоявшаясяСверкаНеПутаетсяСНесогласием(t *testing.T) {
	...
	sourceID := источник(t, gate)
	order := Order{SourceID: sourceID, UnitLabel: "3.1"}
	model := &подставнаяМодель{ответы: []string{годныйОтвет, ответВычитки, "не json вовсе", ответСоседа}}
	runner, jobs, _ := конвейер(t, model, order)
