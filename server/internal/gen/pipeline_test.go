...
// ответСоседа — чем отвечает подставная модель на третьем узле.
//
...
func TestPgСлепаяСверкаОстаётсяСлепой(t *testing.T) {
	...
	// Узлов три: написание, слепая сверка, различающая сверка по
	// единственному соседу круга. Счёт точный, а не «хотя бы два»:
	// исчезнувший узел иначе прошёл бы молча, а сверка, съехавшая на
	// другое место, сверялась бы ниже не с тем текстом.
	if len(model.спрошено) != 3 {
		t.Fatalf("узлов прошло %d вместо трёх", len(model.спрошено))
	}
...
func TestPgНесостоявшаясяСверкаНеПутаетсяСНесогласием(t *testing.T) {
	...
	sourceID := источник(t, gate)
	order := Order{SourceID: sourceID, UnitLabel: "3.1"}
	model := &подставнаяМодель{ответы: []string{годныйОтвет, "не json вовсе", ответСоседа}}
	runner, jobs, _ := конвейер(t, model, order)
