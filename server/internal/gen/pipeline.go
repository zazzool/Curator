...
func (r *Runner) run(ctx context.Context, job Job) (Result, error) {
	...
	if err != nil {
		...
		//
		// Причина, однако, записывается: пустой Verdict говорит «сверки
		// не было» и молчит о том, почему. Сверка не состоится и когда
		// кончились деньги у поставщика, и когда задание отменили, и
		// когда модель вернула не тот JSON, — а на глаз эти три случая
		// выглядят одинаково пустым разбором у сотни задач подряд.
