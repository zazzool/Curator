...
// варианта.
//
// Сверяются варианты ГОТОВОГО черновика без эталона, а не соседи плана
// напрямую: сверять надо то, что реально уйдёт обучающемуся. Вариант, у
// которого положений не нашлось, называется несверенным, а не молчится —
// см. Note.
//
...
func (r *Runner) checkSiblings(ctx context.Context, job Job, draft Draft) []SiblingCheck {
	rivals := draft.Rivals(job.Plan)
	if len(rivals) == 0 {
