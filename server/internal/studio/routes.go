...
func (d *Desk) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Одно и то же сообщение на неизвестное имя, на негодный код, на
		// закрытый вход и на исчерпанные попытки: разные ответы рассказали
		// бы постороннему, какое имя существует и что с ним дальше делать.
		// Один и тот же код ответа — по той же причине: 401 против 429
		// различимы снаружи не хуже слов.
		//
		// Про исчерпанные попытки сказано ровно столько же, сколько про
		// прочее, то есть ничего. Назвать срок запрета значило бы сказать
		// перебирающему, когда возвращаться; честному человеку срок не
		// нужен — до предела он не доходит.
