...
//
// Тридцать суток (решение владельца 20.09.2026). Прежний срок — сутки — был
// выбран из того же довода, что и нынешний, только доведён до конца не был:
// вход, истекающий за час, люди обходят — оставляют вкладку открытой или
// заводят второй способ входа, — и срок, который обходят, не защищает.
// Сутки обходят ровно так же: составитель работает над источником неделями,
// и вход, требующий кода каждое утро, он начинает обходить теми же
// способами. Тридцать суток — срок, который не заставляет искать обход.
//
// Плата названа вслух: украденная сессия живёт месяц, а не день. Дороже это
// ровно настолько, насколько дороже месяц вместо дня, и оплачено тем, что
// отнять сессию можно немедленно: «Выйти» закрывает её на сервере, а
// отключённый пользователь не ходит по выданной ранее сессии вовсе (см.
// User ниже) — так что увольнение действует сразу и при тридцати сутках.
