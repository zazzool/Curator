...
CREATE TABLE IF NOT EXISTS source_documents (
    ...
    -- пойдёт по обеим копиям и даст две редакции одного источника.
    UNIQUE (sha256)
);
...
CREATE TABLE IF NOT EXISTS gen_jobs (
    ...
    -- Ключ повторности: повтор с тем же ключом не заводит второго задания.
    -- Держит это указатель базы, а не проверка перед вставкой: проверка и
    -- вставка — два шага, и соперники сталкиваются между ними.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_gen_jobs_idem
    ON gen_jobs (idem_key) WHERE idem_key <> '';
