...
-- Своё задание узла для одного источника.
--
-- Конвейер у источников один, а задания разные: разбор приказа и разбор
-- клинических рекомендаций читают по-разному, и одно задание на оба
-- пришлось бы писать так обще, что оно перестало бы помогать обоим.
--
-- Отдельной таблицей, а не колонкой source_id у самого задания. Довод
-- не в чистоте: указатель «умолчание ровно одно на узел» уже стоит на
-- живой базе как prompts(node) WHERE is_default, и колонка потребовала
-- бы его заменить — то есть снести и построить заново на таблице, куда
-- в эту же минуту пишет студия. Схема растёт добавлением, и здесь это
-- ровно то же правило.
--
-- Связь ведёт на ЧУЖОЕ задание, а не на общее: укажи источник на само
-- умолчание, и правка «для этого источника» молча меняла бы задание
-- всем. Стережёт это Prompts.Fork — он заводит новую строку, а не
-- связывает со старой.
