...
class ApiFailure implements Exception {
  ...
  /// Устройство больше не опознаётся, и токен с него уже стёрт.
  ///
  /// Отличается от обычного отказа сервера тем, что лечится ровно одним
  /// действием — возвратом прежней записи по почте, — и экран обязан это
  /// действие предложить. Без признака врач читает «Войдите заново» и не
  /// знает, куда именно войти: пароля в продукте нет, а возврат лежит в
  /// трёх нажатиях вглубь настроек.
...
class Api {
  ...
  Future<Map<String, dynamic>> _send(
    ...
    try {
      ...
      //
      // Испорченный токен стирается по той же причине, что и отозванный:
      // непустым он остаётся навсегда, а `ensureEnrolled` при непустом
      // токене устройство не заводит — то есть приложение не чинится ни
      // перезапуском, ни ожиданием.
      await _forgetDevice();
      throw ApiFailure(
        'Устройство не опознано. Войдите заново',
        lostDevice: true,
      );
    }
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      ...
      // Не `text`: так зовётся тело ответа выше, и одноимённая тень в
      // соседних строках читается как одно и то же.
      final message = said is String && said.isNotEmpty
          ? said
          : 'Сервер отказал. Попробуйте позже';

      // Токен, который сервер больше не признаёт, стирается — и это не
      // уборка, а единственный выход. `ensureEnrolled` видит непустой
      // токен и не заводит устройство заново НИКОГДА, поэтому отозванный
      // токен (мошенничество, восстановление базы, смена сборки) оставлял
      // приложение навсегда на «Устройство не опознано»: каждый экран
      // отказывал, перезапуск не менял ничего, помогала одна переустановка
      // — то есть потеря всего, что не привязано к почте.
      //
      // Признак `lostDevice` едет с отказом, чтобы экран предложил
      // ВЕРНУТЬ прежнюю запись, а не просто завёлся заново: заведись он
      // молча при следующем запуске, врач получил бы чистую запись вместо
      // своей и не понял бы, куда делись знаки и купленные наборы.
      //
      // Только у запросов с токеном: 401 без токена — это «код не
      // подошёл» при возврате доступа, и стирать там нечего.
      if (authorized && response.statusCode == HttpStatus.unauthorized) {
        await _forgetDevice();
        throw ApiFailure(
          message,
          status: response.statusCode,
  ...
  /// Забывает токен устройства, чем бы ни кончилась сама уборка.
  ///
  /// Отказ хранилища здесь не должен подменять собой отказ, ради которого
  /// всё и затевалось: врачу надо показать «устройство не опознано», а не
  /// исключение настроек, о котором ему нечего думать.
  Future<void> _forgetDevice() async {
    try {
      await tokens.clear();
    } catch (error) {
      // Молча для врача: ему показывают «устройство не опознано», и это
      // всё, что он может понять и сделать.
      debugPrint('токен не стёрся: $error');
    }
  }
}
