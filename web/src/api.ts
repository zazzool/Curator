...
/**
 * Что известно о слепой сверке черновика.
 *
 * Вопроса два, и ответ на первый не выражается вторым: сверка СОСТОЯЛАСЬ
 * или нет, и если состоялась — сошлась или нет. Нет поля `check` вовсе —
 * значит сверка не дошла даже до попытки; `done: false` — попытка была и
 * не удалась, и `note` говорит почему. Показывать это надо отдельно от
 * несогласия: непроверенная задача, показанная как чистая, обманывает в
 * худшую сторону.
 */
export type Check = {
  done: boolean
  ...
  verdict?: {
    ...
    why: string
    sure: boolean
    /** Сошлась ли сверка с заказанным. Считает сервер, а не модель. */
    agrees: boolean
  }
  /**
   * Разбор расхождения: второй, ЗРЯЧИЙ вопрос по положениям источника.
   * Пусто — не разбирали (сверка сошлась либо черновик старше узла).
   * Стоит рядом с вердиктом, а не вместо: `verdict.agrees` остаётся тем,
   * что ответила слепая сверка.
   */
  arbitration?: {
    /** «ordered» — заказанная точнее, «found» — сосед, «unclear» — не решить. */
    verdict: 'ordered' | 'found' | 'unclear'
    /** Метка соседа, если победил сосед. */
    which?: string
    why: string
  }
  /** Почему разбора не было при расхождении. */
  arbitrationNote?: string
}

/**
 * Итог различающей сверки по одному неверному варианту.
 *
 * Вопросов снова два, и путать их нельзя: `check.done` говорит, сверяли ли
 * вариант вообще, а `confirms` — подтверждает ли условие и его, то есть
 * выходит ли у задачи второй верный ответ. Несверенный вариант — это не
 * чистый вариант.
 *
 * `remark` — готовые слова составителю, и пишет их сервер. Студия
 * показывает их как есть и выбирает по `confirms` и `check.done` только
 * вид полосы: сложи она те же слова сама — и две формулировки одного
 * замечания разошлись бы молча.
 */
...
/** Одна правка редактора: было, стало и — у отклонённых — почему. */
export type ProofreadChange = {
  /** Что правилось: `title` или номер фрагмента (`s1`, `s2`, …). */
  field: string
  before: string
  after: string
  reason?: string
 ...
 * Итог вычитки.
 *
 * `done` отделён от содержимого по тому же доводу, что у сверки:
 * «вычитки не было» и «вычитана, замечаний нет» — разные вещи, и пустой
 * список правок выражает только вторую.
 *
 * Отклонённые правки показываются обязательно: отклонённая правка часто
 * верна по сути и не прошла только по заслону — составитель применит её
 * рукой, если увидит.
 */
export type Proofread = {
  ...
  /** Слова составителю. Их пишет сервер — см. SiblingCheck.remark. */
  remark?: string
}

/** Одна найденная подсказка. Род — закрытый словарь сервера. */
export type Cue = {
  kind: string
  /** Где нашлось: `segments` или `segments[2]`. */
  where: string
  message: string
}

/**
 * Итог детектора подсказок.
 *
 * Исходов три — «чисто», «есть подсказки», «судить не могу», — и
 * различаются они полем `done`, а не уговором о том, как читать пустоту.
 * Пустой список иначе значил бы разом две несовместимые вещи: подсказок
 * нет и мерить было нечем.
 */
export type CueCheck = {
  done: boolean
  note?: string
  cues?: Cue[]
  /** Слова составителю. Их пишет сервер — см. SiblingCheck.remark. */
...
/** Одна находка судьи: чьё правило сработало и что оно нашло. */
...
/**
 * Итог судьи — машинных проверок свода по готовому черновику.
 *
 * Исходов не два, а четыре, и путать их нельзя. Поля нет вовсе — судья не
 * ходил. `done: false` — ходил и судить не смог. `done: true` при
 * `checked: 0` — ходил, но ни одно действующее правило машинной проверки
 * не несёт, то есть чистота тут не доказана, а просто не проверялась.
 * И только `done: true` при `checked > 0` с пустыми находками значит
 * «прогнали и чисто».
 */
export type RuleCheckResult = {
  done: boolean
  note?: string
  /** Сколько правил с проверками прогнано. */
  checked: number
 ...
 */
/**
 * Машинная проверка правила: предикат из закрытого каталога.
 *
 * Правило с проверкой и правило без неё держатся по-разному — первое
 * обязательно к исполнению, второе зависит от сговорчивости модели, — и
 * различие это составитель должен видеть, решая, чинить задачу или
 * правило.
 *
 * Заполняется она формой по каталогу, а не набирается текстом: каталог
 * закрыт, и свободное поле отдало бы серверу предикат, которого нет, —
 * человек узнал бы об этом после заполнения всей формы.
 */
...
export type Rule = {
  ...
  pinned: boolean
  scope: RuleScope
  /** Машинная проверка. Пусто — правило держится одним текстом в задании. */
  check?: RuleCheck | null
  /** Проверка словами. Пишет её сервер — см. kindWord. */
  checkWords: string
...
export type RuleEdit = {
  ...
  /**
   * Проверка, и состояний у поля ТРИ, а не два.
   *
   * Поля нет — проверку не трогали; `null` — снимают; запись — ставят.
   * Различает их сервер, и потому поле необязательное: пошли студия
   * `null` вместо «нет поля», и правка формулировки снимала бы проверку
   * молча.
   */
  check?: RuleCheck | null
}

/** Предикат каталога: что он такое и какие у него доводы. */
export type CheckSpec = {
  type: string
  title: string
  about: string
  params: string[]
  /** Предикат недоступен модели: пишет его только составитель. */
  doctorOnly?: boolean
}

/** Запись закрытого словаря: что уедет на сервер и как это зовётся. */
...
/**
 * Каталог проверок со словарями их доводов.
 *
 * Приходит с сервера целиком, и студия его не повторяет: повторённый
 * разошёлся бы молча — ровно на том предикате или значении, которое
 * добавили последним, — и составитель выбрал бы то, чего исполнитель не
 * знает.
 */
...
export type Draft = {
  ...
  check?: Check
  /**
   * Итоги различающей сверки. Поля нет вовсе — сверки не было; ПУСТОЙ
   * СПИСОК — сверять было нечего. Разные случаи: у первого задачу никто
   * не смотрел на второй верный ответ, у второго смотреть было не на что.
   */
  siblings?: SiblingCheck[]
  /** Итог вычитки. Поля нет — вычитки не было вовсе. */
  proofread?: Proofread
  /** Итог детектора подсказок. Поля нет — детектор не дошёл. */
  cues?: CueCheck
...
export const api = {
  ...
  checkCatalog: () => request<CheckCatalog>('GET', '/admin/api/rules/checks'),
