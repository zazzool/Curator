...
function CheckNote({ check }: { check?: Check }) {
  ...
  // Расхождение разобрано вторым, зрячим вопросом — и разбор решил, что
  // заказанное точнее. Такая задача исправна, и держать её нельзя: слепая
  // сверка рассудила верно, а отметила не тот вариант. Тревога при этом
  // не прячется, а объясняется: составитель должен видеть, что спор был.
  const разбор = check.arbitration
  if (разбор?.verdict === 'ordered') {
    return (
      <Banner kind="info">
        Слепая сверка выбрала «{verdict.answer}», но разбор по положениям
        источника признал заказанный ответ точнее. {разбор.why} Похоже, сверка
        рассудила верно и отметила не тот вариант — задача исправна.
      </Banner>
    )
  }
  if (разбор?.verdict === 'found') {
    return (
      <Banner kind="error">
        Слепая сверка НЕ сошлась, и разбор по положениям источника её
        подтвердил: условие отвечает
        {разбор.which ? ` соседней единице «${разбор.which}»` : ' не заказанной единице'}
        . {разбор.why} Это уже не промах сверки — посмотрите условие.
      </Banner>
    )
  }
  return (
    <Banner kind="error">
      ...
      {разбор?.verdict === 'unclear' && (
        <> Разбор по положениям источника решить не смог: {разбор.why}</>
      )}
      {/*
        Несостоявшийся разбор называется вслух. Молчание составитель примет
        за «разобрали и подтвердили» — то есть за более тяжёлую новость,
        чем есть, и разбираться он пойдёт не с тем.
      */}
      {check.arbitrationNote && (
        <> Разобрать расхождение не удалось: {check.arbitrationNote}.</>
      )}
    </Banner>
  )
}
